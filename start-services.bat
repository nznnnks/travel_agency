...
start "Trip Service" cmd /k "cd /d %~dp0trip-service && ..\mvnw.cmd spring-boot:run"

echo.
echo Starting Review Service on port 8082...
start "Review Service" cmd /k "cd /d %~dp0review-service && ..\mvnw.cmd spring-boot:run"

echo.
echo Starting Match Service on port 8083...
start "Match Service" cmd /k "cd /d %~dp0match-service && ..\mvnw.cmd spring-boot:run"
